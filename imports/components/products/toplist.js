...
class Toplist extends React.Component {
  constructor(props) {
    ...
    this.getAffiliateToplist = this.getAffiliateToplist.bind(this);
   ...
   */
  getStatus(point) {
    const { status } = this.state.toplist || {};
    let statusObj;

    if (status && status.length) {
      let minPoints = (status || []).map(prod => (parseInt(prod.minPoints) || 0), 10);

      minPoints = (minPoints || []).sort((a, b) => a - b);
