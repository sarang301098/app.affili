...
const cacheAnalysis = (stepResults) => {
  ...
  if (!cache.has(platform, stepResults.url, stepResults.userLocale, stepResults.hostname)) {
    stepResults.isFromCache = false;
    logger.debug('cache missed: url:', platform, stepResults.userLocale, stepResults.url);
    return;
  }

  const cached = cache.get(platform, stepResults.url, userLocale, stepResults.hostname);
