...
const canEnableCache = (routePattern, routes, statusCode) => {
  // cache everything except 302 (temp redirects)
  return statusCode !== 302 && process.env.NODE_ENV === 'production';
  ...
  nextTick(() => {
    ...
    if (canEnableCache(routePattern, routes, statusCode)) {
      ...
      if (statusCode === 301) {
        ...
        cache.setTempRedirect(url, Location);
      } else {
        cache.setPage(userAgent, userLocale, url, html, hash, statusCode, hostname);
      }
    } else {
      logger.debug('!canEnableCache');
    }
  });
};
